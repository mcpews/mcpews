...
class Encryption {
    ...
    initializeCipher (secretKey, salt) {
        let key = hashBuffer(hashAlgorithm, Buffer.concat([ salt, secretKey ]));
        let initialVector = key.slice(0, blockSize);
...
class ServerEncryption extends Encryption {
    ...
    completeKeyExchange (clientPubKeyStr) {
        let clientPubKey = asNodejsPubKey(Buffer.from(clientPubKeyStr, "base64"));
...
class ClientEncryption extends Encryption {
    ...
    completeKeyExchange (serverPubKeyStr, saltStr) {
        let serverPubKey = asNodejsPubKey(Buffer.from(serverPubKeyStr, "base64"));
        let salt = Buffer.from(saltStr, "base64");
